...
class RunningSessionViewModel @Inject constructor(
        ...
        viewModelScope.launch {
            ...
            intent {
                ...
                repeat(INITIAL_COUNTDOWN) {
                    reduce {
                        state.copy(
                            state = RunningSessionState.MainReady(
                ...
                reduce {
                    state.copy(
                        state = RunningSessionState.MainRunning(
                                ...
                                state.copy(
                                    state = currentState.copy(
                                        mapUiState = currentState.mapUiState.copy(
                                            paceColors = currentState.mapUiState.paceColors
                                                .dropLast(1)
                                                .toMutableList()
                                                .apply {
                                                    add(newPaceColors)
                                                },
                                            path = currentState.mapUiState.path
                                                .dropLast(1)
                                                .toMutableList()
                                                .apply {
                                                    add(newPath)
    ...
    private fun handlePause() {
        intent {
            ...
            if (currentState is RunningSessionState.MainRunning) {
                reduce {
                    state.copy(
                        state = RunningSessionState.MainPause(
                            mapUiState = currentState.mapUiState.copy(
            ...
            intent {
                ...
                if (currentState is RunningSessionState.MainPause) {
                    ...
                    repeat(INITIAL_COUNTDOWN) {
                        reduce {
                            state.copy(
                                state = RunningSessionState.CoolDownReady(
                                    mapUiState = currentState.mapUiState,
                                    countdown = INITIAL_COUNTDOWN - timer
                                )
                            )
    ...
    private fun calculatePace(totalTimeInSeconds: Int, distanceInKm: Double): String {
